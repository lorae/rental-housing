...
**A note on data use and restrictions**: The conclusions in this report are based exclusively on publicly available data from the Public Use File (PUF), along with its associated codebooks, methodology reports, and supporting documentation. A more detailed version of the dataset, the Internal Use File (IUF), is restricted to use only by those with explicit data partnerships with the U.S. Census Bureau to prevent unauthorized disclosures of property-specific information.

While both the PUF and IUF contain the same number of property-level observations, the IUF includes additional variables that are either suppressed, topcoded, bottomcoded, or recoded in the PUF to protect respondent confidentiality. For example, geographic identifiers such as census tracts are removed from the PUF. However, the survey codebooks provide a complete listing of variables included in both versions, allowing public users to identify the additional data fields present in the IUF, even without direct access to the restricted survey responses.

As such, the response rate concerns outlined in this report apply to the survey in general, whether it's viewed through the restricted-use or public-use datasets.
...
![table02](supporting_files/table02.png)
...
**Table 4: 2018 RHFS Sample Size by Stratum**
...
Applying the interview rate to completed interviews, I estimate that (4,331 / 0.643) 6736 BSAs were eligible for interview.
...
**Table 5: 2021 RHFS Sample Size by Stratum**
